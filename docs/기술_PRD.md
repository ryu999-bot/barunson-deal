...
User { email, name, provider('email'|'google'), role, pw?, phone?, bizNo?, certName?,
       vendorAccess, vendorName?, request?, profile? }
...
### 3.2 다지점(직영·가맹) 정산
- **전제:** 전 지점 동일 상품·동일가 → 쿠폰은 전 지점 사용 가능. 구매 시 대표 지점(`branchId`) 기록.
- **귀속:** 정산 라인은 **사용처리한 지점**(`usedBranchId`/`history[].branchId`)에 귀속. 구매 지점 아님.
- **지급:** 직영점 사용분 → 본사 계좌 합산, 가맹점 사용분 → **각 가맹점 계좌 직접 지급**(본사 경유 재분배 없음). 세금계산서 사업자별.
- 산출 함수: `domain.groupByPayee(settleLines)`. 지점 데이터 `config.BRANCHES`.
...
| 현행(목) | 실제 연동 |
|----------|-----------|
| `api.listCoupons()` | `GET /partner/v1/vouchers` (바른손 발급·상태 동기화) |
| `api.redeem(code,branchId)` / `deduct(code,amount,branchId)` | `POST /partner/v1/vouchers/{code}/redeem` · `/deduct` (**멱등키 필수**, 409=이미사용, 422=잔여초과) |
...
| `auth.*` | `POST /auth/*` (세션/JWT), Google ID 토큰 서버 검증, `POST /vendor-access`(+MID 매핑) |
...
- 바른라운지 = **사용 기준 70/30 홀드백**(익월 25일, 다지점 귀속). **모델이 근본적으로 다름.**
