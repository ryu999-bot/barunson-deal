...
- **수수료 공제:** 위 산정액은 **공제 전(gross)**. 실지급액 = gross − 판매수수료(요율 원천 = 백오피스 /Company 업체 마스터 → ERP). 파트너 화면·명세서는 gross/수수료/net 구분 표기(기능_PRD F-05). 현 프로토타입은 gross만 표시 — 연동 시 서버 정산 응답에 수수료 포함 필요.
