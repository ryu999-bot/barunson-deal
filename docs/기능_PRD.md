...
| **바른손카드 통합관리자** (bbarunsonweb) | **제휴사(업체) 등록·수수료율** 마스터 (상품등록은 하지 않음) |
| **바른손카드 어드민** (admin.barunsoncard.com) | **이용권(딜) 상품등록 — "부속품"으로 등록**(유형·판매가·유효기간·판매 지점) → 사이트 전시 |
...
```
① 제휴사(업체) 등록·수수료율 = 통합관리자(bbarunsonweb) / 이용권(딜) 상품등록 = 바른손카드 어드민(admin.barunsoncard.com, "부속품"으로 등록)
...
- **만료 임박 자동 알림:** 유효기간 도래 전(D-30 필수, D-7 권장) 잔액·만료일·환불(환급) 방법 안내 발송.
- **마이페이지 환불 신청:** 미사용·부분사용 쿠폰의 환불 신청 버튼. **유효기간 연장은 제공하지 않음**(2026-07-21 정책 확정 — 연장은 강행규정 아님, 만료 후 90% 환급으로 갈음).
...
- **운영 구분(2026-07-22 확정):** **본사(계약 업체) 계정 = 바른손카드가 발급해 전달**(계약 시), **가맹점(별도 사업자) = 자가 회원가입 + 승인**. 두 경로 모두 시스템 지원.
...
- **횟수형(count):** 사용 회수 입력(기본 1회) 후 **차감**(잔여 회수·회차 내역 표시, 잔여 0이면 소멸). **구매자 본인 전용 — 가족·지인과 잔여 회수를 나누어 사용 불가**(2026-07-22 확정, 금액형 동일).
