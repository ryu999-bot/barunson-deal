...
② 바른라운지 파트너 페이지 — 로그인 계정 발급(직원 콘솔, 자체 관리)
        ↓ (계정 ↔ 사업자 ↔ 지점 매핑)
③ 바른라운지 파트너 페이지 — 지점 등록(API: POST /api/Lounge/branches)
        → 등록 지점 = 사용처리 "처리 지점" = 정산 귀속 단위
```

- **제휴사·딜·수수료율 마스터 = 통합관리자**, **로그인 계정·지점 등록 = 파트너 페이지**로 역할이 확정됨.
...
- **비밀번호 정책(연동 시):** 초기 비밀번호는 최초 로그인 시 변경 강제, 분실 시 재설정(직원 콘솔 초기화 또는 고객센터 경유). 서버는 해시 저장.

### F-02 계정 발급 (직원 콘솔) — 어드민 자체 관리 (2026-07-08 핸드오버 확정)
- **로그인 계정 관리는 파트너 페이지 자체 담당**(바른손카드 API 범위 아님). 직원 콘솔에서 사업자명·사업자번호·담당자·아이디·초기 비밀번호·운영 지점(복수 선택)을 입력해 발급, 목록 조회·**회수**(즉시 로그인 차단).
- 단, **제휴사(업체) 자체와 수수료율의 마스터 = 통합관리자**(§2.3 ①) — 계정 발급 전 통합관리자에 제휴사가 먼저 등록되어 있어야 하며, 계정은 그 제휴사(정산주체)와 매핑된다.
- 계정↔사업자↔지점 매핑이 판매분·정산 연동의 확정점.
