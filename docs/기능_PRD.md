...
- **바른손카드 직원(staff):** 전체 매출/사용률/정산 모니터링, 업체 권한 승인, 공지 발행.
- **핵심 연결:** ① 회원 계정 ↔ 업체(가맹점) 매핑(권한 승인) ② 사용 기준 정산 ③ 바른손카드 본사 시스템과의 **쿠폰 발급·사용·정산 실시간 연동**.
...
- **권한 게이팅:** 업체는 `vendorAccess=granted` 이전까지 운영 메뉴(판매현황·사용처리·문자이력·정산·명세서) 잠금.
- **역할 게이팅:** 직원 전용 화면(운영 대시보드·직원 콘솔)은 vendor 접근 차단.
- **(연동 후) 지점 계정 3층:** 바른손카드 직원 / 브랜드 관리자(본사) / 지점 계정(branchId 고정) — 지점 로그인 시 처리 지점 자동 고정, 자기 지점 데이터만 조회.

## 3. 기능 요구사항

### F-01 인증/계정
- 이메일 회원가입: 담당자명·이메일·**비밀번호(8자+특수문자)+확인**·연락처·**사업자등록번호**·**사업자등록증 첨부**.
- 이메일 로그인, **Google 로그인(직원 전용, role=staff)**, 세션 유지, 로그아웃.
...
- **처리 지점 선택(다지점):** 쿠폰은 전 지점 사용 가능. 사용처리·차감 시 처리 지점 기록 → 정산 귀속 기준. 연동 시 지점 계정별 자동 지정.
...
- 유형별 정산액 산정: [기술_PRD §5.1](기술_PRD.md) 참조.
