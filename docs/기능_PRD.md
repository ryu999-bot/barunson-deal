...
### F-02 계정 발급 (직원 콘솔)
- **계정 생성의 단일 원천 = 바른손 백오피스** ([bbarunsonweb /Company](https://bbarunsonweb.barunsoncard.com/Company)) — 업체 등록·계정·수수료율 마스터, 바른 ERP와 연동(§2.3). 파트너 페이지 계정은 이 업체 마스터와 매핑(단방향: /Company → ERP → 파트너).
- 파트너 직원 콘솔의 발급 폼(사업자명·사업자번호·담당자·아이디·초기 비밀번호·운영 지점 복수 선택)은 **마스터 연동 전(M0) 임시 수단**이며, 연동 후에는 **조회·회수(즉시 로그인 차단)·비밀번호 초기화**만 수행한다.
- 계정↔사업자↔지점 매핑이 판매분·정산 연동(MID 연결)의 확정점.
